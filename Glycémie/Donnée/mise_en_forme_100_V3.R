...
data_denivele <- read_excel("Données_course -psycho-alim/denivele100_par_km.xlsx")
...
data_glycemie %<>% filter(course == "100 km")
...
data_chrono <- read_excel("Chrono-course-Labo-Ut4M2023.xlsx", sheet = "Labo tps passage 100M")


# Suppresion des colonnes dont nous n'allons pas nous servir 
data_chrono <- data_chrono[,-c(2,3,4,5,6,9,10,11,13,14,15,16,17,18,19)]
...
data_chrono %<>%
  mutate(heure_ecoule = ifelse(section == "Recoin",0, heure_ecoule))

# Date de départ
date_depart <- as.POSIXct("2023-07-22 07:00:00")
...
data_score %<>%
  ...
  mutate(temps_ecoule_en_heure = ifelse(section == "Recoin", 0, difftime(date, lag(date), units = "hours")))
...
data_score %<>%
  ...
  mutate(date_reference = case_when(
    section == "Recoin" ~ as.POSIXct("2023-07-22 07:00:00", format = "%Y-%m-%d %H:%M:%S"),
...
data_km_section <- data.frame("km"= c(0:97))

data_km_section %<>% 
  mutate(section = case_when(
    km == 0 ~ "Recoin",
    km > 0 & km <= 22 ~ "Croix de Chamrousse",
    km >= 23 & km <= 56 ~ "St- Nazaire-les Eymes",
    km >= 57 & km <= 69 ~ "Cabane du Bachasson",
    km >= 70 & km <= 97 ~ "Grenoble",
    TRUE ~ NA_character_
  )) %>% 
  mutate(section_psy = case_when(
    km >= 0 & km <= 22 ~ "Montee 1",
    km >= 32 & km <= 56 ~ "Descente 1",
    km >= 57 & km <= 69 ~ "Montee 2",
    km >= 90 & km <= 97 ~ "Descente 2",
    TRUE ~ NA_character_
  ))
...
data_km_section_score_denivele %<>%
  mutate(
    ...
    nouvelle_date = as.POSIXct("2023-07-22 07:00:00", format = "%Y-%m-%d %H:%M:%S") + lubridate::seconds(temps_en_seconde_cumule)
  ) %>%
