...
# data2$ITRA_rating=as.numeric(data2$ITRA_rating)

# Tableau selon l'expérience en trail
data2$start_trail_gp=data2$start_trail
# data2$start_trail_gp=forcats::fct_na_value_to_level(data2$start_trail_gp)

ITRA = data2%>% tbl_summary(by = start_trail_gp, missing_text = "(Données manquantes)",
                            label = list(sex ~ "Sexe",
                                         age ~ "Age",
                                         weight ~ "Poids",
...
data <- read_excel("../../Data/Reponse_questionnaire_V2.xlsx")
datatest=data
datatest$which_races_UT4M_2023

library(stringr)
g=str_detect(datatest$which_races_UT4M_2023, ",")
