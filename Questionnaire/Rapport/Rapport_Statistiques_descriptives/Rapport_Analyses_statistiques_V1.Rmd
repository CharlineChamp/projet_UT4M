...
# Course effectuée
Puis, nous avons réalisé une analyse statistique des différentes variables selon la course effectuée.
```{r}
...
data_test=data%>%select(!which_races_UT4M_2023)
data_test_20=data_test%>%select(!c(race_40,race_80,race_100,race_160))

for (i in 1:length(data_test_20$race_20)){
  if(data_test_20$race_20[i]==0){
    data_test_20$race_20[i]="Un des 20km"
  }
  else{
    data_test_20$race_20[i]=NA
  }
}
data_test_20=data_test_20%>%filter(!is.na(race_20))
data_test_20=rename(data_test_20,"race"="race_20")


data_test_40=data_test%>%select(!c(race_20,race_80,race_100,race_160))
for (i in 1:length(data_test_40$race_40)){
  if(data_test_40$race_40[i]==0){
    data_test_40$race_40[i]="Un des 40km"
  }
  else{
    data_test_40$race_40[i]=NA
  }
}
data_test_40=data_test_40%>%filter(!is.na(race_40))
data_test_40=rename(data_test_40,"race"="race_40")



data_test_80=data_test%>%select(!c(race_20,race_40,race_100,race_160))
for (i in 1:length(data_test_80$race_80)){
  if(data_test_80$race_80[i]==0){
    data_test_80$race_80[i]="80km Challenge"
  }
  else{
    data_test_80$race_80[i]=NA
  }
}
data_test_80=data_test_80%>%filter(!is.na(race_80))
data_test_80=rename(data_test_80,"race"="race_80")

data_test_100=data_test%>%select(!c(race_20,race_40,race_80,race_160))
for (i in 1:length(data_test_100$race_100)){
  if(data_test_100$race_100[i]==0){
    data_test_100$race_100[i]="100km Master"
  }
  else{
    data_test_100$race_100[i]=NA
  }
}
data_test_100=data_test_100%>%filter(!is.na(race_100))
data_test_100=rename(data_test_100,"race"="race_100")




data_test_160=data_test%>%select(!c(race_20,race_40,race_80,race_100))
for (i in 1:length(data_test_160$race_160)){
  if(data_test_160$race_160[i]==0){
    data_test_160$race_160[i]="160km Xtrem / 160km Challenge"
  }
  else{
    data_test_160$race_160[i]=NA
  }
}
data_test_160=data_test_160%>%filter(!is.na(race_160))
data_test_160=rename(data_test_160,"race"="race_160")


df_final=rbind(data_test_20,data_test_40,data_test_80,data_test_100,data_test_160)

data3 = df_final[-c(1,7,10,11,27,28,30,32,34,37,38,42,43)]

data3$sex <- as.factor(data3$sex)
data3$weekly_volume_profession <- as.factor(data3$weekly_volume_profession)
data3$weekly_volume_profession <- ordered(data3$weekly_volume_profession,
                                         levels = c("⩽ 35 heures / semaine" ,
                                                "entre 36 et 45 heures / semaine",
                                                    "⩾ 45h / semaine" ))
data3$start_trail <- as.factor(data3$start_trail)
data3$start_trail <- ordered(data3$start_trail, 
                            levels = c("< 2 ans"  ,
                                       "Entre 2 et 5 ans",
                                       "Entre 5 et 10 ans",
                                       "> 10 ans"))
data3$time_proportion_other_practice <- as.factor(data3$time_proportion_other_practice)
data3$time_proportion_other_practice <- ordered(data3$time_proportion_other_practice, 
                                               levels = c("< 20%",
                                                          "Entre 20% et 50%",
                                                          "> 50%"))
data3$coach_for_trail = as.factor(data3$coach_for_trail)
data3$heart_rate_monitor_train = as.factor(data3$heart_rate_monitor_train)
data3$heart_rate_monitor_competition = as.factor(data3$heart_rate_monitor_competition)
data3$connected_platform = as.factor(data3$connected_platform)
data3$ITRA_rating_know = as.factor(data3$ITRA_rating_know)
data3$number_hours_practice_trail_phase_preparation = as.factor(data3$number_hours_practice_trail_phase_preparation)
data3$number_hours_practice_trail_phase_preparation = factor(data3$number_hours_practice_trail_phase_preparation,levels=c(
  '⩽ 3 heures / semaine','Entre 3h et 7h / semaine','Entre 7h et 12h / semaine','> 12 heures / semaine'))
data3$drop_practice_trail_phase_preparation = as.factor(data3$drop_practice_trail_phase_preparation)
data3$drop_practice_trail_phase_preparation = factor(data3$drop_practice_trail_phase_preparation,levels=c('< 1000m / semaine','Entre 1000m et 2000m / semaine', '> 2000m / semaine'))
data3$kilometres_practice_trail_phase_preparation = as.factor(data3$kilometres_practice_trail_phase_preparation)
data3$kilometres_practice_trail_phase_preparation = factor(data3$kilometres_practice_trail_phase_preparation,levels=c(
  '< 20km / semaine','Entre 20km et 50km / semaine','Entre 50km et 80km / semaine','> 80km / semaine'))
data3$number_hours_training_trail_phase_preparation = as.factor(data3$number_hours_training_trail_phase_preparation)
data3$number_hours_training_trail_phase_preparation = factor(data3$number_hours_training_trail_phase_preparation,levels=c(
  '⩽ 3 heures / semaine','Entre 3h et 7h / semaine','Entre 7h et 12h / semaine','> 12 heures / semaine'))
data3$trail_participation_last_two_seasons=as.factor(data3$trail_participation_last_two_seasons)
data3$distance_trail_participation_last_two_seasons=as.factor(data3$distance_trail_participation_last_two_seasons)
data3$distance_trail_participation_last_two_seasons=fct_relevel(data3$distance_trail_participation_last_two_seasons, c("< 40km", "De 40km à 80km", "> 80km"))
data3$frequence_trail_participation_last_two_seasons_by_season=as.factor(data3$frequence_trail_participation_last_two_seasons_by_season)
data3$frequence_trail_participation_last_two_seasons_by_season=fct_relevel(data3$frequence_trail_participation_last_two_seasons_by_season, 
                                                                         c("1 à 3 courses / saison", "4 à 8 courses / saison", "> 8 courses / saison"))
data3$diet=as.factor(data3$diet)
data3$volume_drink_during_training=as.factor(data3$volume_drink_during_training)
data3$volume_drink_during_competition=as.factor(data3$volume_drink_during_competition)
data3$frequence_hours_eat_during_training <- as.factor(data3$frequence_hours_eat_during_training)
data3$frequence_hours_eat_during_training <- ordered(data3$frequence_hours_eat_during_training, 
                                                    levels = c("< 1x / heure",
                                                               "1x ou 2x / heure",
                                                               "> 2x / heure"))
data3$frequence_hours_eat_during_competition <- as.factor(data3$frequence_hours_eat_during_competition)
data3$frequence_hours_eat_during_competition <- ordered(data3$frequence_hours_eat_during_competition, 
                                                       levels = c("< 1x / heure",
                                                                  "1x ou 2x / heure",
                                                                  "> 2x / heure"))
data3$trail_injury_illness <- as.factor(data3$trail_injury_illness)
data3$trail_injury_illness <- ordered(data3$trail_injury_illness, 
                                     levels = c("Oui",
                                                "Non"))
data3$trail_health_problem_involve_no_training <- as.factor(data3$trail_health_problem_involve_no_training)
data3$trail_health_problem_involve_no_training <- ordered(data3$trail_health_problem_involve_no_training, 
                                                         levels = c("Oui",
                                                                    "Non"))
data3$time_trail_health_problem_involve_no_training <- as.factor(data3$time_trail_health_problem_involve_no_training)
data3$time_trail_health_problem_involve_no_training <- ordered(data3$time_trail_health_problem_involve_no_training, 
                                                              levels = c("< 2 semaines",
                                                                         "2 à 4 semaines",
                                                                         "5 à 8 semaines",
                                                                         "> 8 semaines"))
data3$frequence_medical_apointment_trail <- as.factor(data3$frequence_medical_apointment_trail)
data3$frequence_medical_apointment_trail <- ordered(data3$frequence_medical_apointment_trail, 
                                                   levels = c("< 2x / an",
                                                              "2x à 6x / an",
                                                              "> 6x / an"))
data3$smoke = as.factor(data3$smoke)
data3$race=fct_relevel(data3$race, c("Un des 20km","Un des 40km","80km Challenge","100km Master","160km Xtrem / 160km Challenge"))

datarace= data3
datarace$race_gp = datarace$race
datarace = datarace[-52]

RACE = datarace%>% tbl_summary(by = race_gp, missing_text = "(Données manquantes)",
                                    label = list(sex ~ "Sexe",
                                                 age ~ "Age",
                                                 weight ~ "Poids",
                                                 max_weight ~ "Poids maximal",
...
```
Concernant le sexe, nous pouvons remarquer que les femmes sont plus présentes sur un des 40 km. Les hommes sont plus présents sur les plus longues distances. Les participants du 160km sont plus vieux que que les autres courses. 

Globalement, les coureurs inscrits sur les 20 et 40km ont commencé le trail il y a moins de 2 ans et en moyenne, les coureurs inscrits sur les 160km ont débuté il y a plus de 10 ans.

Ceux qui ont une meilleure côte ITRA ont participé aux courses 100 et 160 km. Les coureurs ayant participé aux longues distances ont une côte ITRA plus élevée.

Les coureurs qui effectuent majoritairement plus de 2000m de dénivelé par semaine ont participé au 100 et 160km et ceux qui grimpent moins de 1000 m par semaine sont pour la plupart inscrit à un 40 ou un 20 km.

Les coureurs ayant couru un 100 et 160km ont presque tous participé à une ou plusieurs courses de trail lors des 2 dernières saisons.

Globalement, les coureurs ayant fait moins de 40km lors des dernières courses de trail sont inscrits sur un des 20, un des 40 ou sur le 80 et ceux ayant couru plus de 80km lors des dernières courses de trail sont inscrits sur le 100 ou un des 160 km.
