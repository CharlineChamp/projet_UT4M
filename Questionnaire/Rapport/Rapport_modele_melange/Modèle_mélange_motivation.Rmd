...
data_motivation <- data[,c(1,48:52)] 

# Sélectionner uniquement les lignes complètes
df <- data_motivation[complete.cases(data_motivation), ]
```
...
model1 <- Mclust(df[,c(2:6)])
#summary(model1)

model2 <- Mclust(df[,c(2:6)], G=2)
#summary(model2)

model3 <- Mclust(df[,c(2:6)], G=3)
#summary(model3)

model4 <- Mclust(df[,c(2:6)], G=4)
#summary(model4)

model5 <- Mclust(df[,c(2:6)], G=5)
#summary(model5)
...
```{r}
data_complete <- data %>% left_join(df[, c(1,7)], by = join_by(file_number))
